...
class Game(ShowBase):
    def __init__(self):
        ...
        self.sprint_speed = 25.0  # Скорость бега
        ...
        self.jump_combo_time = 5.0  # Время в секундах для комбо прыжков
        self.jump_combo_multiplier = 1.0  # Текущий множитель скорости
        self.max_combo_multiplier = 3.0  # Максимальный множитель
        self.last_jump_time = 0  # Время последнего прыжка
        self.combo_task = None  # Задача для отслеживания времени комбо
    ...
    def start_jump(self):
        ...
        if not self.is_jumping:
            # Увеличиваем множитель комбо при последовательных прыжках
            current_time = time.time()
            if current_time - self.last_jump_time < self.jump_combo_time:
                self.jump_combo_multiplier = min(self.jump_combo_multiplier + 0.5, self.max_combo_multiplier)
            else:
                self.jump_combo_multiplier = 1.0

            # Применяем множитель к силе прыжка
            self.vertical_velocity = self.jump_power * self.jump_combo_multiplier
            
            # Добавляем горизонтальный импульс в направлении движения
            move_vec = Vec3(0, 0, 0)
            ...
            if move_vec.length() > 0:
                ...
                # Устанавливаем горизонтальную скорость прыжка
                jump_horizontal_speed = 30.0 if self.is_sprinting else 20.0
                self.horizontal_velocity = Vec3(x, y, 0) * jump_horizontal_speed * self.jump_combo_multiplier
            else:
    ...
    def update(self, task):
        ...
        # Применяем текущий масштаб времени
        ...
        # Обрабатываем движение с учетом замедления времени
        if self.keyMap["w"]: move_vec.setY(move_vec.getY() + 1)
        if self.keyMap["s"]: move_vec.setY(move_vec.getY() - 1)
        if self.keyMap["a"]: move_vec.setX(move_vec.getX() - 1)
        if self.keyMap["d"]: move_vec.setX(move_vec.getX() + 1)
        ...
        if move_vec.length() > 0:
            ...
            # Применяем движение с учетом замедления времени
            if self.keyMap["shift"]:
                move_speed = self.sprint_speed * self.current_time_scale
            else:
                move_speed = self.move_speed * self.current_time_scale

            # Применяем множитель скорости от прыжка
            move_speed *= self.jump_speed_boost
            
            self.camera.setPos(
                self.camera.getX() + x * move_speed * scaled_dt,
                self.camera.getY() + y * move_speed * scaled_dt,
                self.camera.getZ()
            )
        
        # Обрабатываем прыжок и гравитацию с учетом замедления времени
        if self.is_jumping:
            ...
            # Обновляем горизонтальную скорость прыжка
            self.camera.setX(self.camera.getX() + self.horizontal_velocity.getX() * scaled_dt)
            self.camera.setY(self.camera.getY() + self.horizontal_velocity.getY() * scaled_dt)
        ...
        current_speed = math.sqrt(move_vec.getX()**2 + move_vec.getY()**2) * move_speed * self.current_time_scale if move_vec.length() > 0 else 0
