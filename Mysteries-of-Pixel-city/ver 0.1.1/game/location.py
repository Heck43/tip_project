...
class DialogueSystem:
    ...
    def finish_response(self):
        ...
        self.active = False
        self.showing_response = False
        self.choices = ["Спросить о предмете", "Спросить о том, кто она", "Пока, мне нужно идти"]

    def get_response(self, choice):
        responses = {
            "Спросить о предмете": "Ты поймешь это, яркое свечение этого предмета покажет тебе путь.",
            "Спросить том, кто она": "Это сейчас вообще не важно. Держите глаза открытыми.",
...
def main():
    ...
    initial_dialogue_list = [
        ("АААА ты кто вообще?", 50, default_font, False),
        ("Я чувствую, что что-то назревает.", 45, loud_font, True),
        ("Чего? Как ты появилась?", 50, default_font, False),
        ("Быстрее беги на центральную площадь", 45, loud_font, True),
        ("Что? Какая площадь?", 50, default_font, False),
        ("Найди что-то, что поможет остановить ЭТО", 45, loud_font, True),
        ("Ладно дай штаны надеть", 50, default_font, False),
    ]
    ...
    # Устанавливаем шрифт для задания
    task_font = pygame.font.Font('font/better-vcr_0.ttf', 30)
    task_text = task_font.render("Найти ЭТО!", True, (255, 255, 255))
    ...
    while running:
        ...
        if dialogue_system.active or dialogue_system.choice_mode:
         ...
         # Отрисовка текста задания в правом верхнем углу
        screen.blit(task_text, (screen_width - task_text.get_width() - 10, 10))  # 10 пикселей от правого края и верхнего края

        if not initial_dialogue_active and not task_visible:
            task_visible = True  # Устанавливаем флаг, что задание теперь видно

    # Отрисовка текста задания в правом верхнем углу, если задание активно
        if task_visible:
            screen.blit(task_text, (screen_width - task_text.get_width() - 10, 10))  # 10 пикселей от правого края и верхнего края
